<div class="container text-center <?php if ($data['sort'] == 0) {
                                        echo "product_list";
                                    } ?>">
    ...
    if (isset($_GET['search_name'])) {
        ...
        //使用第一層類別查詢
        $queryFirst = sprintf("SELECT * FROM product,product_img,pyclass WHERE p_open=1 AND product_img.sort=1 AND product.p_id=product_img.p_id AND product.classid=pyclass.classid AND pyclass.uplink='%d' ORDER BY product.p_id ", $_GET['classid']);
    } elseif (isset($_GET['classid'])) {
        //使用產品類別查詢
        $queryFirst = sprintf("SELECT * FROM product,product_img,pyclass WHERE p_open=1 AND product_img.sort=1 AND product.p_id=product_img.p_id AND product.classid=pyclass.classid AND product.classid='%d' ORDER BY product.p_id DESC", $_GET['classid']);
    } else {
        //列出產品product資料查詢
        $queryFirst = sprintf("SELECT * FROM product,product_img,pyclass WHERE p_open=1 AND product_img.sort=1 AND product.p_id=product_img.p_id AND product.classid=pyclass.classid ORDER BY product.p_id ");
    }
    ...
    <?php if ($pList01->rowCount() != 0) { ?>
        <?php while ($pList01_Rows = $pList01->fetch()) { ?>
            <?php if ($i % 3 == 1) { ?>
                ...
                <div class="col-md-4 cpl-12">
                    <div class="card productbutton">
                        ...
                        <div class="card-body productcard">
                            ...
                            <p class="card-text productintro"><?php echo mb_substr($pList01_Rows['p_intro'], 0, 50, "utf-8"); ?></p><br>
                            <h4 class="card-text productprice">$<?php echo number_format($pList01_Rows['p_price']); ?><?php echo $pList01_Rows['p_set']; ?></h4>
